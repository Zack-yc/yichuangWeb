...
async function confirmGameStart() {
    ...
    // 显示欢迎提示
    showToast(`🎉 欢迎 ${playerName} 加入遗体捐献协调中心！`);
...
function updateUI() {
    ...
    document.getElementById('resource-time').textContent = gameState.resources.time || 0;
    document.getElementById('resource-fund').textContent = gameState.resources.fund || 0;
    document.getElementById('resource-manpower').textContent = gameState.resources.manpower || 0;
    ...
    document.getElementById('stats-themes').textContent = (gameState.completedThemes ? gameState.completedThemes.length : 0) + '/6';
...
function showInitialTaskModal() {
    ...
    taskCard.innerHTML = `
        ...
        <div style="display: flex; gap: 15px; font-size: 0.85rem; opacity: 0.8;">
            <span>📖 阅读材料</span>
            <span>📝 ${pendingTask.questions.length}道测试题</span>
        </div>
    `;
...
function showEnding() {
    ...
    let totalScore = 0;
    totalScore += satisfaction * 0.5; // 满意度占50%（核心指标）
    totalScore += Math.min(reputation, 100) * 0.25; // 声望占25%
    totalScore += Math.min(manpower * 3, 30) * 0.15; // 人力占15%（影响活动规模）
    totalScore += Math.min(fund / 200, 20) * 0.1; // 资金占10%（弱化，仅基础保障）
    ...
    // 生成数据可视化图表HTML
    const chartsHTML = generateEndingCharts(satisfaction, reputation, manpower, fund, gameState.stats.contribution, completedThemes, totalScore);
    
    // 生成工作报告内容
    const workSummary = generateWorkSummary(endingRank, satisfaction, reputation, manpower, gameState.stats.contribution, completedThemes);
...
function applyEffects(effects) {
    ...
    // 应用统计效果
    if (effects.satisfaction !== undefined) gameState.stats.satisfaction += effects.satisfaction;
    if (effects.contribution !== undefined) gameState.stats.contribution += effects.contribution;
    // 确保资源不为负
...
function applyTimePressureEffects() {
    ...
    // 时间紧张时，轻微降低满意度获取效率
    if (timeRatio < 0.3) {
        // 时间少于30%时，满意度获取效率降低10%
        gameState.stats.satisfaction = Math.max(0, gameState.stats.satisfaction - 0.5);
    } else if (timeRatio < 0.5) {
        // 时间少于50%时，满意度获取效率降低5%
        gameState.stats.satisfaction = Math.max(0, gameState.stats.satisfaction - 0.2);
    }
    // 时间充足时（>50%），无负面影响
}
